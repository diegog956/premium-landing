...
Treat this as an open conversational block, not a one-pass checklist item. Once enough strategic signal exists, read [original-one-shot.md](references/original-one-shot.md) and run two parallel tracks unless the operator explicitly opts out or has already imposed an authoritative fidelity target: research external directions and build one original reference-free landing to production-ready local quality.

Present a separate **Visual options to choose** block with three ranked premium templates or implemented examples and at most one external wildcard, plus the completed **Original one-shot** as a distinct first-party option. Prioritize current external candidates from the curated discovery sources in [reference-research.md](references/reference-research.md), link to their actual previews, and include screenshots when practical. For the original option, return the verified local URL and current desktop/mobile screenshots; never substitute a moodboard, hero-only proof, static mockup, design rationale, or unfinished prototype. Clearly distinguish live sites, archived references, free templates, paid templates, and the locally implemented original.
...
Close the block only when the operator selects the original one-shot, selects an external base and optional section-level influences, delegates the final choice, explicitly chooses no external reference, or requests uninterrupted execution. Selecting the original makes its internally authored system the visual authority. Selecting an external option preserves the original in Git and returns to the normal fidelity-led proof workflow. In delegated mode, compare the completed original with the external evidence, choose the strongest direction, and record why. Never force a weak template merely to close the block.
...
Read [creative-orchestration.md](references/creative-orchestration.md) and [build-and-qa.md](references/build-and-qa.md). Resolve creative authority and complete the anti-slop preflight before implementation. In the normal fidelity-led track, first create only the design proof: header, hero, and one representative section using the real visual system, content, and approved assets. Do not build the remaining page until this proof passes its blocking visual gate and the operator approves it or has delegated uninterrupted execution. In the original one-shot track, run that proof and gate internally, continue immediately to the complete landing, and expose nothing until the full production-ready local gate in [original-one-shot.md](references/original-one-shot.md) passes.
...
- Use `design-taste-frontend` v2 as a blocking anti-slop challenger before coding and again against the rendered design proof. It is not a second art director: apply only contextual rules, preserve the approved Astro architecture, and never let its default fonts, frameworks, dials, or aesthetic families override the brief, brand kit, or visual contract.
- Do not use `high-end-visual-design` as a default premiumizer. Its recipes may be used only when the approved direction independently calls for them; never introduce pills, bento grids, glass, double bezels, oversized radii, or generic luxury typography merely because that skill suggests them.
...
- After a static design proof exists, use `find-animation-opportunities` to identify and reject motion candidates. Implement only accepted, purposeful motion using `emil-design-eng`, then run `review-animations` as a blocking rendered-motion gate on the proof and the completed page. Use `improve-animations` only for systemic failures that need a prioritized remediation plan, not as routine ornamentation.
- Run `humanizer` on final customer-facing copy after factual and conversion review, in audit-only mode. Report credible clusters of AI patterns with excerpts, severity, and an optional correction proposal. Never let `humanizer` rewrite copy directly. For agent-generated, unapproved original-one-shot copy, route blocking findings back through `copywriting` and re-audit; never modify user-supplied or approved copy without explicit permission. Preserve meaning and brand voice, and never invent personality, claims, opinions, or deliberate mistakes merely to sound human.
